...
const BlogItem = ({ title, description, category, image, id }) => {
    return (
        <div className='max-w-[330px] sm:max-w-[300px] bg-white border border-black hover:shadow-[-7px_7px_0px_#000000] transition-all duration-300 ease-in-out'>
            ...
            <p className='ml-5 mt-5 px-2 py-1 rounded-sm inline-block bg-black text-white text-sm'>{category}</p>
            <div className='p-5'>
                ...
                <Link href={`/blogs/${id}`} className='inline-flex items-center py-2 font-semibold text-center'>
                    Read more <Image src={assets.arrow} className='ml-2' alt='arrow-logo' width={12} height="auto" />
                </Link>
            </div>
        </div>
    )
